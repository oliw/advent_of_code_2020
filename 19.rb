...
# I think I want it to be something you push and pop stuff from

parts = input.split("\n\n")
raw_rules = parts[0].split("\n")
...
raw_rules.each do |r|
  ...
  when /"\w"/
    # terminal
    rules[parent] = raw_child[1]
  when /\|/
    # branch
    children = []
    raw_children = raw_child.split(" | ")
    raw_children.each do |c|
      children << c.split(" ").map(&:to_i)
    end
    rules[parent] = children
  else
    child = raw_child.split(" ").map(&:to_i)
    rules[parent] = [child]
  end
end

def is_match?(message, rules, stack, depth)
  buffer = "-"*depth
  return true if message == "" && stack.empty?
  return false if message == "" && !stack.empty?
  head = message[0]
  rest = message[1..]
  current_rule = stack.pop
  next_rules = rules[current_rule]
  if next_rules.is_a? String
    if head == next_rules
      return is_match?(rest, rules, stack, depth+1)
    else
      return false
    end
  else
    return next_rules.any? do |next_rs|
      new_stack = stack.dup
      next_rs.reverse.each do |rule|
        new_stack.push(rule)
      end
      is_match?(message, rules, new_stack, depth+1)
    end
  end
end

puts "Part 1"
matches = messages.select {|m| is_match?(m, rules, [0], 0) }
puts matches.sort
puts matches.length
