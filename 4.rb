...
def valid_passport?(passport)
  expected_entries = %w(byr iyr eyr hgt hcl ecl pid)
  seen = Hash.new
  expected_entries.each {|e| seen[e] = false}

  parts = passport.split(/\s/)
...
def valid_passport_part2?(passport)
  ...
  rules["hgt"] = ->(x) do
    matches = x.match(/^(?<number>\d+)(?<unit>(cm)|(in))$/)
  ...
  rules["ecl"] = ->(x) do
    return %w(amb blu brn gry grn hzl oth).include? x
  end
  rules["pid"] = ->(x) do
    /^\d{9}$/.match? x
  end
  ...
  parts.each do |part|
    prefix = part.split(":")[0]
    value = part.split(":")[1]
    seen[prefix] = rules[prefix].call(value)
  end
...
puts "Part 2"
puts passports.count {|passport| valid_passport_part2?(passport)}
