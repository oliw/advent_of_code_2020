...
def find_closing_bracket(str, start_i)
  i = start_i+1
  opened_brackets = 1
  while (i < str.length)
    current_token = str[i]
...
def solve(math)
  ...
  i = 0
  while i < math.length
    current_token = math[i]
    # puts "current: #{current_token} tally: #{tally} operation: #{operation}"
    case current_token
    when " "
      # Do nothing
      i+=1
    ...
    when "*"
      ...
      i+=1
    when "("
      j = find_closing_bracket(math, i)
      sub_math = math[(i+1)...(j)]
      number = solve(sub_math)
      tally = tally.send(operation, number)
      i = j+1
    else
    ...
    # puts "next_tally: #{tally}"
  end
  tally
end

puts "Part 1"
puts inputs.map {|i| solve(i)}.sum
...
### Today I learned / had to look up
### Today I remembered that arithemtic expressions can be solved using stacks
### Two stage process -> First convert to postfix notation
### Then solve
### i couldn't remember the formula for converting to postfix notation
