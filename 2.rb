...
PART_1_PATTERN_REGEX = /^(?<min>\d+)-(?<max>\d+) (?<letter>[a-z]): (?<password>.+)$/

# PART 1
def part1_valid?(row)
  matches = row.match(PART_1_PATTERN_REGEX)
  min = matches[:min].to_i
  max = matches[:max].to_i
  letter = matches[:letter]
  password = matches[:password]
  num_occurences = password.split("").count{|e| e == letter}
  min <= num_occurences && num_occurences <= max
end

num_valid = input.count{|row| part1_valid?(row)}
puts "Part 1 ----"
puts "#{num_valid}"

# PART 2
PART_2_PATTERN_REGEX = /^(?<index1>\d+)-(?<index2>\d+) (?<letter>[a-z]): (?<password>.+)$/

def part2_valid?(row)
  matches = row.match(PART_2_PATTERN_REGEX)
  index1 = matches[:index1].to_i - 1
  index2 = matches[:index2].to_i - 1
  letter = matches[:letter]
  password = matches[:password]
  (password[index1] == letter) ^ (password[index2] == letter)
end

num_valid = input.count{|row| part2_valid?(row)}
puts "Part 2 ----"
puts "#{num_valid}"