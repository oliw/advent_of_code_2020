...
foods = foods.map do |raw_food|
  first_bracket_idx = raw_food.index("(")
  raw_ingredients = raw_food[0...first_bracket_idx]
  ingredients = raw_ingredients.split(" ")

  raw_allergens = raw_food[(first_bracket_idx+1+9)...-1]
  allergens = raw_allergens.split(", ")

  Food.new(ingredients, allergens)
end

# Find the ingredients that definitely don't contain an allergen

allergens = Hash.new {|h,k| h[k] = []}
foods.each do |food|
  food.allergens.each do |allergen|
    if allergens[allergen].empty?
      allergens[allergen] = food.ingredients
    else
      allergens[allergen] = allergens[allergen] & food.ingredients
    end
  end
end

all_ingredients = foods.flat_map(&:ingredients).uniq
ingredients_that_could_be_allergens = allergens.values.flatten.uniq

ingredients_that_cant_be_allergens = all_ingredients-ingredients_that_could_be_allergens

puts "Part 1"
freq_count = 0
foods.each do |food|
  freq_count+= food.ingredients.count {|i| ingredients_that_cant_be_allergens.include?(i)}
end
puts freq_count


bad_allergens = Hash.new
while allergens.length > 0
  # find the value thats just length 1
  found_allegen = nil
  ...
  ingredient_list = allergens.delete(found_allegen)
  found_ingredient = ingredient_list[0]
  allergens.transform_values! {|v| v - ingredient_list}
  bad_allergens[found_allegen] = found_ingredient
end

puts "Part 2"
puts bad_allergens.to_a.sort_by(&:first).map(&:last).join(",")