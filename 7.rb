...
def get_parent_color(rule)
	matches = /(?<color>.* .*) bags contain.*/.match(rule)
	matches[:color]
end

parents_hash = Hash.new {|h,k| h[k] = []}

rules.each do |rule|
	matches = /(?<color>.* .*) bags contain (?<rest>.*)/.match(rule)
	parent = matches[:color]
	rest = matches[:rest]
	next if rest == "no other bags."
	raw_children = rest.split(", ")
	raw_children.each do |raw_child|
		child_pattern = /\d+ (?<color>.* .*) bags?/
		child = child_pattern.match(raw_child)[:color]
		parents_hash[child] << parent # maybe dedup
	end
end

def can_contain(hash, bag)
	possible_parents = hash[bag]
	return [] if possible_parents.empty?
	possible_parents.each do |p|
		puts "#{bag} can go into #{p}"
	end
	possible_parents + (possible_parents.map {|p| can_contain(hash, p)}).flatten.uniq
end

puts "Part 1"
puts can_contain(parents_hash, "shiny gold").length

Content = Struct.new(:number, :color)
...
rules.each do |rule|
	...
	raw_children = rest.split(", ")
	raw_children.each do |raw_child|
		child_pattern = /(?<number>\d+) (?<color>.* .*) bags?/
		matches = child_pattern.match(raw_child)
		color = matches[:color]
		number = matches[:number].to_i
		content = Content.new(number, color)
		children_hash[parent] << content # maybe dedup
	end
end

def num_bags(hash, color)
	children = hash[color]
	return 1 if children.empty?

	sum = 1
	children.each do |child|
		sum += child.number * num_bags(hash, child.color)
	end
	sum
end

puts "Part 2"
puts num_bags(children_hash, "shiny gold") - 1